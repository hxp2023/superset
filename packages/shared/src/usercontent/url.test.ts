...
const BASE = "https://pages.supersetusercontent.com";
...
describe("pageViewUrl", () => {
	...
	test("pinned version, ticketed: relative references inherit the prefix", () => {
		...
		expect(url).toBe(
			`https://${ID}.pages.supersetusercontent.com/versions/3/~${TICKET}/`,
		);
...
describe("pageThumbnailUrl", () => {
	test("ticket stays in the query", () => {
		expect(
			...
			`https://${ID}.pages.supersetusercontent.com/versions/2/thumbnail.jpg?ticket=${encodeURIComponent(TICKET)}`,
		);
	});

	test("public thumbnail is bare", () => {
		expect(pageThumbnailUrl({ baseUrl: BASE, pageId: ID, version: 2 })).toBe(
			`https://${ID}.pages.supersetusercontent.com/versions/2/thumbnail.jpg`,
		);
	});
});

describe("pageIdFromHost", () => {
	const baseHost = "pages.supersetusercontent.com";
