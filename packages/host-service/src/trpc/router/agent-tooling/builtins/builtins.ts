...
function builtin(name: string, description: string): SlashCommand {
	return {
		name,
		aliases: [],
		description,
...
export const CLAUDE_BUILTIN_SLASH_COMMANDS: readonly SlashCommand[] = [
	builtin("add-dir", "Add a new working directory"),
	builtin("agents", "Manage agent configurations"),
	builtin("clear", "Start a new session with empty context"),
	builtin("compact", "Compact the conversation to free context"),
	...
	builtin("model", "Change the model for this session"),
	builtin("permissions", "Manage tool permissions"),
	builtin("resume", "Resume a previous conversation"),
	builtin("review", "Review changes for bugs and issues"),
...
export const CODEX_BUILTIN_SLASH_COMMANDS: readonly SlashCommand[] = [
	...
	builtin("mcp", "List configured MCP tools"),
	...
	builtin("usage", "View account usage or use a usage limit reset"),
