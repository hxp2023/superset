...
/**
 * Collapse replicated `(scope, tag)` rows for rendering. Local wins when it
 * has a value; otherwise the lexicographically first host wins so the result
 * cannot change with query completion order.
 */
